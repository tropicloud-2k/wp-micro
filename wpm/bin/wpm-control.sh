...
wpm_start() {	
	...
	if [[  ! -z $MEMCACHE_PORT || ! -z $REDIS_PORT  ]]; then
		if [[  ! -z $REDIS_PORT  ]]; then echo -e "  Redis listening \033[0;37m@\033[0m $WPM_REDIS"; fi
		if [[  ! -z $MEMCACHE_PORT  ]]; then echo -e "  Memcached listening \033[0;37m@\033[0m $WPM_MEMCACHE \n"; fi
	fi
